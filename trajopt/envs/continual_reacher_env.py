...
class ContinualReacher7DOFEnv(Reacher7DOFEnv):
    ...
    def _step(self, a):
        ...
        # if (self.env_timestep % (n + self.count) == 0 or (dist < 0.10 and not self.train)) and not (target_pos.sum() == 0):
        if (self.env_timestep % (n + self.count)) == 0 and not (target_pos.sum() == 0):
            done = True
        else:
            done = False
